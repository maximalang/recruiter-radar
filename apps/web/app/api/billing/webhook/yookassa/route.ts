...
export async function POST(request: Request) {
  ...
  try {
    // The adapter verifies the notification by requesting the payment from
    // YooKassa with this shop's own credentials before changing an order.
    ...
    await pool.query(
      ...
      [
        ...
        processed ? null : result.body.slice(0, 500),
        claimToken,
      ]
    );
    ...
    return NextResponse.json(
      { ok: false, error: "reconciliation_error" },
      { status: 500 }
    );
  }
}
