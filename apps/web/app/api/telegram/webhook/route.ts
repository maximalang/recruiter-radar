...
import { sanitizeTelegramError } from "../../../../lib/telegram/sanitize-error";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await pool.query(`UPDATE webhook_events SET status = 'failed', processed_at = NOW(), error_message = LEFT($2, 1000) WHERE id = $1 AND processing_claim_token = $3`, [eventRow.id, sanitizeTelegramError(message), claimToken]);
