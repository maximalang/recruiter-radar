...
export async function POST(request: Request) {
  ...
  const claim = await pool.query<{ id: number; status: string; canProcess: boolean }>(`
    INSERT INTO webhook_events (
      provider, event_type, external_event_id, idempotency_key, payload, status, processing_claimed_at
    )
    VALUES ('telegram', 'callback_query', $1, $2, $3::jsonb, 'processing', NOW())
    ON CONFLICT (provider, idempotency_key)
    DO UPDATE SET
      payload = EXCLUDED.payload,
      processing_claimed_at = CASE
        WHEN webhook_events.status IN ('received', 'failed')
          OR (webhook_events.status = 'processing' AND webhook_events.processing_claimed_at < NOW() - ($4::int * INTERVAL '1 second'))
        THEN NOW()
        ELSE webhook_events.processing_claimed_at
      END,
      status = CASE
        WHEN webhook_events.status IN ('received', 'failed')
          OR (webhook_events.status = 'processing' AND webhook_events.processing_claimed_at < NOW() - ($4::int * INTERVAL '1 second'))
    ...
    RETURNING id, status,
      (
        status = 'processing'
        AND processing_claimed_at > NOW() - INTERVAL '3 seconds'
      ) AS "canProcess"
  `, [callbackQueryId ?? idempotencyKey, idempotencyKey, JSON.stringify(body), STALE_CLAIM_SECONDS]);

  const eventRow = claim.rows[0];
  if (!eventRow.canProcess) return NextResponse.json({ ok: true, replaySafe: true, duplicate: true });

  if (!callbackQueryId || !parsedCallback) {
    await pool.query(`UPDATE webhook_events SET status = 'ignored', processed_at = NOW() WHERE id = $1`, [eventRow.id]);
  ...
  try {
    ...
    await pool.query(`UPDATE webhook_events SET status = 'processed', processed_at = NOW(), error_message = NULL WHERE id = $1`, [eventRow.id]);
    ...
    await pool.query(`UPDATE webhook_events SET status = 'failed', processed_at = NOW(), error_message = LEFT($2, 1000) WHERE id = $1`, [eventRow.id, sanitizeError(message)]);
