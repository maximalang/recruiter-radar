    ...
    <PageFrame maxWidth="820px">
      <section className={s.docSection}>
        ...
        <div className={s.docSummary}>
          ...
          <Summary label="Аналитика" value="Только по согласию (Метрика и собственная телеметрия)" />
        ...
        <div className={s.docBody}>
          ...
          <PolicySection n="2" title="Необязательная аналитика: Яндекс Метрика и собственная телеметрия">
            <p><strong>Назначение:</strong> обезличенная оценка посещаемости и спроса на публичных страницах (какие разделы читают, откуда приходят посетители, какие источники приводят к заявкам). Метрика не размещается на checkout, в аккаунте, onboarding и операционных разделах.</p>
            ...
            <p><strong>Собственная телеметрия:</strong> после согласия сайт также отправляет обезличенные события (просмотры и взаимодействия с лендингом) в свою систему. Мы не храним IP-адрес, сырые строки Referer и User-Agent — только широкие категории источника и устройства, а также псевдонимный идентификатор визита, который меняется ежедневно и не связан с аккаунтом. События хранятся не дольше 24 месяцев.</p>
            <div className={s.docTableWrap}>
              <table className={s.docTable}>
