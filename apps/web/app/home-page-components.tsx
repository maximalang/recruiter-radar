...
export function buildFaqItems(paymentConfigured: boolean) {
  return [
    ...
    {
      question: "Чем Recruiter Radar отличается от базы вакансий?",
      answer: "База показывает публикации. Recruiter Radar сопоставляет сигнал с профилем агентства, проверяет компанию и актуальность момента, применяет confidence gate и объясняет, почему этой компании стоит написать сейчас. Исходные факты, даты и источники остаются рядом с выводом."
    },
    {
      question: "Можно ли настроить конкретные роли, отрасли и регионы?",
      answer: "Да. В профиле задаются специализация, целевые роли, отрасли, география и исключения. Приоритеты пересчитываются под этот профиль, поэтому два агентства могут получить разный порядок компаний из одного набора сигналов."
    },
    {
    ...
    {
      question: "Что происходит после окончания пилота?",
      answer: paymentConfigured
        ? "После окончания пилота доступ не продлевается автоматически и деньги не списываются. Если радар оказался полезен, можно отдельно подключить месяц или квартал; если нет — ничего отменять не нужно."
        : "После окончания пилота доступ не продлевается автоматически. Продолжение на месяц или квартал подключается отдельно, когда оплата снова доступна."
    },
    {
      question: "Как обрабатываются данные?",
      answer: "Для выдачи используются данные компании, публичные корпоративные каналы и настройки профиля агентства. Мы не собираем персональные контакты для массовой рассылки, не продаём базы и не отправляем сообщения компаниям автоматически."
    }
  ] as const;
}
