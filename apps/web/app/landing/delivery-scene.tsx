...
type DeliveryChannel = { key: DeliveryChannelKey; title: string; status: string; text: string };
...
const PRIMARY_ROUTES: ReadonlyArray<DeliveryChannel> = [
  { key: "telegram", title: "Telegram", status: "Можно подключить", text: "Короткое уведомление о новом приоритетном сигнале." },
  { key: "email", title: "Email", status: "Можно подключить", text: "Подборка новых возможностей на рабочую почту." },
];

const EXTRA_ROUTES: ReadonlyArray<DeliveryChannel> = [
  { key: "vk", title: "VK", status: "Можно подключить", text: "Уведомления через рабочее сообщество." },
  { key: "push", title: "Push в браузере", status: "Можно подключить", text: "Короткие браузерные уведомления." },
...
function DeliveryChannelGlyph({ channel }: { channel: DeliveryChannelKey | "cabinet" }) {
  ...
  if (channel === "email") return <svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3.5" y="6" width="17" height="12" rx="1.5" stroke="currentColor" strokeWidth="1.35" /><path d="m5 8 7 5 7-5" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" strokeLinejoin="round" /></svg>;
