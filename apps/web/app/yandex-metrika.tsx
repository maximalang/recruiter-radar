...
export default function YandexMetrika() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Storage can be unavailable in privacy modes. Keep analytics disabled.
    }
  }, []);
  ...
  function decide(next: Exclude<AnalyticsConsent, "pending">) {
    try {
      ...
      // The decision still applies for the current page if storage is blocked.
    }
    setConsent(next);
  }
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gap: 5 }}>
        <strong style={{ fontSize: 15 }}>Помогите улучшать Recruiter Radar</strong>
        <span style={{ fontSize: 13, lineHeight: 1.5, color: "#475569" }}>
          Мы можем включить обезличенную Яндекс Метрику только на публичной странице.
          Личный кабинет, настройки радара и платёжные данные в аналитику не передаются. {" "}
