...
jest.mock('@/lib/db-pool', () => ({ getClient: jest.fn() }))
...
function request(
  ...
  options: { token?: string; modern?: boolean; protocol?: string; origin?: string; realIp?: string } = {},
) {
  ...
  } else if (options.protocol) headers['mcp-protocol-version'] = options.protocol
...
function accessToken(overrides: Record<string, unknown> = {}) {
  const header = encodeJson({ alg: 'ES256', typ: 'JWT', kid: KID })
  ...
  const client = {
    query: jest.fn(async (sql: string) => {
      if (sql.includes("current_setting('server_version')")) {
        return { rows: [{ serverVersion: '16.9', inRecovery: false, transactionReadOnly: 'on', serverTime: '2026-08-12T09:00:00.000Z' }] }
      }
      if (sql.includes("TO_REGCLASS('public.schema_migrations')")) return { rows: [{ present: true }] }
      if (sql.includes('FROM schema_migrations')) {
        return { rows: [{ migrationCount: '42', latestMigration: '20260809140000_add_query_plan_quality_feedback_v2', latestAppliedAt: '2026-08-12T08:00:00.000Z' }] }
      }
      return { rows: [] }
    }),
    release: jest.fn(),
...
describe('private operator MCP route', () => {
  const originalEnv = {
    ...
    subjects: process.env.RR_MCP_ALLOWED_SUBJECTS,
  ...
  beforeEach(() => {
    ...
    process.env.RR_MCP_ALLOWED_SUBJECTS = SUBJECT
  ...
  afterAll(() => {
    ...
    restore('RR_MCP_ALLOWED_SUBJECTS', originalEnv.subjects)
  ...
  it('publishes exact RFC 9728 metadata for the read-only rollout', async () => {
    for (const response of [await GET_PATH_PROTECTED_RESOURCE(), await GET_COMPAT_PROTECTED_RESOURCE()]) {
  ...
  it('validates exact issuer, single resource audience, immutable owner and token time claims', async () => {
    expect((await POST(request(rpc('tools/list'), { token: accessToken() }))).status).toBe(200)
    expect((await POST(request(rpc('tools/list'), { token: accessToken({ iss: 'https://attacker.example/operator/oauth' }), realIp: '203.0.113.11' }))).status).toBe(401)
    expect((await POST(request(rpc('tools/list'), { token: accessToken({ aud: 'https://attacker.example/mcp' }), realIp: '203.0.113.12' }))).status).toBe(401)
    expect((await POST(request(rpc('tools/list'), { token: accessToken({ aud: [OPERATOR_MCP_RESOURCE, 'https://attacker.example/mcp'] }), realIp: '203.0.113.13' }))).status).toBe(401)
    expect((await POST(request(rpc('tools/list'), { token: accessToken({ sub: 'rr_other' }), realIp: '203.0.113.14' }))).status).toBe(401)
    expect((await POST(request(rpc('tools/list'), { token: accessToken({ exp: NOW_SECONDS - 300 }), realIp: '203.0.113.15' }))).status).toBe(401)
    expect((await POST(request(rpc('tools/list'), { token: accessToken({ iat: NOW_SECONDS + 300 }), realIp: '203.0.113.16' }))).status).toBe(401)
  })

  it('uses path-specific OAuth challenge and 403 for insufficient read scope', async () => {
    const missing = await POST(request(rpc('tools/list')))
    expect(missing.status).toBe(401)
    expect(missing.headers.get('www-authenticate')).toContain('/.well-known/oauth-protected-resource/api/internal/mcp')
    const insufficient = await POST(request(rpc('tools/list'), { token: accessToken({ scope: 'openid' }), realIp: '203.0.113.17' }))
  ...
  it('exposes only bounded read tools by default', async () => {
    ...
    expect(names).toEqual([
      'get_production_state', 'get_system_health', 'get_service_state', 'get_recent_logs',
      'get_resource_usage', 'get_reverse_proxy_state', 'get_database_state',
      'get_quality_validation_state', 'list_quality_review_targets',
    ])
    ...
    for (const tool of body.result.tools) {
      expect(tool.securitySchemes).toEqual([{ type: 'oauth2', scopes: [OPERATOR_MCP_READ_SCOPE] }])
      expect(tool.annotations.readOnlyHint).toBe(true)
    }
  })

  it('keeps mutations hidden by default and requires distinct scopes behind the explicit gate', async () => {
    expect(getOperatorMcpTools().map((tool) => tool.name)).not.toContain('restart_service')
    process.env.RR_MCP_MUTATIONS_ENABLED = 'true'
    const list = await POST(request(rpc('tools/list'), { token: accessToken({ scope: `${OPERATOR_MCP_READ_SCOPE} ${OPERATOR_MCP_RESTART_SCOPE} ${OPERATOR_MCP_PROXY_SCOPE}` }) }))
    ...
    expect(restart.securitySchemes[0].scopes).toEqual([OPERATOR_MCP_READ_SCOPE, OPERATOR_MCP_RESTART_SCOPE])
    expect(proxy.securitySchemes[0].scopes).toEqual([OPERATOR_MCP_READ_SCOPE, OPERATOR_MCP_PROXY_SCOPE])
    expect(restart.inputSchema.properties.service.enum).toEqual(['web', 'n8n'])
    expect(restart.annotations.readOnlyHint).toBe(false)
    const denied = await POST(request(rpc('tools/call', { name: 'restart_service', arguments: { service: 'web', idempotencyKey: 'restart:test:0001' } }), { token: accessToken(), realIp: '203.0.113.20' }))
  ...
  it('rejects untrusted browser origins while allowing ChatGPT origin', async () => {
    expect((await POST(request(rpc('tools/list'), { token: accessToken(), origin: 'https://evil.example' }))).status).toBe(403)
    expect((await POST(request(rpc('tools/list'), { token: accessToken(), origin: 'https://chatgpt.com', realIp: '203.0.113.21' }))).status).toBe(200)
  })

  it('supports current stateless discovery and only the bounded legacy fallback', async () => {
    const discovery = await POST(request(rpc('server/discover'), { token: accessToken() }))
    expect(discovery.status).toBe(200)
    expect((await discovery.json()).result.supportedVersions).toEqual([OPERATOR_MCP_PROTOCOL_VERSION, OPERATOR_MCP_LEGACY_PROTOCOL_VERSION])
    const legacy = await POST(request(rpc('initialize', { protocolVersion: OPERATOR_MCP_LEGACY_PROTOCOL_VERSION, capabilities: {}, clientInfo: { name: 'test', version: '1.0.0' } }), { token: accessToken(), modern: false }))
    expect(legacy.status).toBe(200)
    const obsolete = await POST(request(rpc('initialize', { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'obsolete', version: '1.0.0' } }), { token: accessToken(), modern: false, realIp: '203.0.113.22' }))
    expect(obsolete.status).toBe(400)
    const missingHeader = await POST(request(rpc('tools/list'), { token: accessToken(), modern: false, realIp: '203.0.113.23' }))
  ...
  it('requires current routing headers to match JSON-RPC', async () => {
    ...
    const mismatched = new NextRequest(req.url, { method: 'POST', headers, body: JSON.stringify(body) })
    expect((await POST(mismatched)).status).toBe(400)
  })

  it('runs DB diagnostics read-only and emits sanitized audit', async () => {
    const client = mockReadOnlyClient()
    const audit = jest.spyOn(console, 'info').mockImplementation(() => undefined)
    const response = await POST(request(rpc('tools/call', { name: 'get_database_state', arguments: {} }), { token: accessToken(), realIp: '203.0.113.24' }))
    expect(response.status).toBe(200)
    const body = await response.json()
  ...
  it('rate limits authenticated buckets independently', () => {
    resetOperatorMcpSecurityCachesForTests()
    const now = Date.now()
    for (let index = 0; index < OPERATOR_MCP_RATE_LIMIT; index += 1) expect(checkOperatorMcpRateLimit('sub:owner', now).allowed).toBe(true)
