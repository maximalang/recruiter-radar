...
describe('BrandLogo', () => {
  it('renders the new radar mark with a separate wordmark', () => {
    const { container } = render(<BrandLogo />);

    const logo = screen.getByRole('img', { name: 'Recruiter Radar' });
    const artwork = logo.querySelector('img');

    expect(artwork?.getAttribute('src')).toBe('/brand/recruiter-radar-mark.svg?v=brand-8');
    expect(artwork?.getAttribute('width')).toBe('64');
    expect(artwork?.getAttribute('height')).toBe('64');
