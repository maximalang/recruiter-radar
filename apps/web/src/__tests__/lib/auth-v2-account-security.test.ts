...
describe("auth v2 account security", () => {
  ...
  test("requires explicit deletion text and leaves purge scheduling unset by default", async () => {
    ...
    expect(allSql).toContain("UPDATE client_profiles");
    expect(allSql).toContain("delivery_enabled = FALSE");
    expect(allSql).toContain("UPDATE notification_provider_accounts");
    expect(allSql).toContain("UPDATE notification_endpoints");
    expect(allSql).toContain("UPDATE notification_routes");
    expect(allSql).toContain("UPDATE notification_delivery_jobs");
