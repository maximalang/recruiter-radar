...
const mockReadAuthV2Cookie = jest.mocked(readAuthV2SessionCookie);
...
const mockRequireWorkspacePermission = jest.mocked(
  requireWorkspacePermission,
);
...
describe("Auth v2 authorization DAL", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockReadAuthV2Cookie.mockResolvedValue(null);
    ...
    mockRequireWorkspacePermission.mockResolvedValue(workspace);
  });

  test("separates the actor from the workspace data owner and enforces permission", async () => {
    mockReadAuthV2Cookie.mockResolvedValue("v2-token");
  ...
  test("never falls back to a legacy identity when a v2 cookie is invalid", async () => {
    mockReadAuthV2Cookie.mockResolvedValue("invalid-v2-token");
  ...
  test("never falls back to a legacy identity when the v2 cookie cannot be read", async () => {
    mockReadAuthV2Cookie.mockRejectedValue(new Error("cookie read failed"));
    mockReadOwnerSession.mockResolvedValue("99");
  ...
  test("fails closed on a denied workspace permission without legacy fallback", async () => {
    mockReadAuthV2Cookie.mockResolvedValue("v2-token");
    mockReadAuthSession.mockResolvedValue(authSession);
    mockRequireWorkspacePermission.mockRejectedValue(new Error("denied"));
    mockReadOwnerSession.mockResolvedValue("99");
