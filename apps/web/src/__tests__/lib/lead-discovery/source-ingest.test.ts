...
describe('source-ingest', () => {
  ...
  describe('company-site targets FILE (live-public from DB orgs the radar tracks)', () => {
    ...
    it('derives COMPANY_SITE_TARGETS_FILE from orgs with a domain + a hiring signal, writes the file', async () => {
      ...
      expect(targetQuery?.[0]).toContain('FROM org_source_refs AS domain_ref_owner')
      expect(targetQuery?.[0]).toContain('domain_ref_owner.org_id <> orgs.id')
      expect(targetQuery?.[0]).toContain("domain_ref_owner.source_key = 'domain:' ||")
      expect(targetQuery?.[0]).toMatch(/FROM org_source_refs AS domain_ref_owner[\s\S]*COALESCE\(NULLIF\(BTRIM\(orgs\.domain\), ''\), NULLIF\(BTRIM\(orgs\.website_url\), ''\)\)/)
