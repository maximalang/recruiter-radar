...
describe('T2.1: qualityMetrics computed from real data', () => {
  it('completeness < 1 when enrichment is sparse', () => {
    const lead: MultiSourceLead = {
      ...
      score: 2.5,
      confidence: 'B',
      sources: [{ sourceId: 'hh', confidence: 0.7, extractedAt: new Date() }],
      signals: [],
      nextAction: 'review',
      reasons: [],
      detectedAt: new Date(),
  ...
  it('completeness = 1 when all enrichment fields are filled', () => {
    const lead: MultiSourceLead = {
      ...
      confidence: 'A',
      sources: [
        { sourceId: 'career-pages', confidence: 0.9, extractedAt: new Date() },
        { sourceId: 'hh', confidence: 0.7, extractedAt: new Date() },
        { sourceId: 'rabota-rossii', confidence: 0.6, extractedAt: new Date() },
      ],
      signals: [],
      nextAction: 'outreach',
      reasons: [],
