...
describe('production scheduler authority', () => {
  ...
  it('fails Source Refresh on a real partial failure and logs only safe counters', () => {
    ...
    expect(sourceRefreshWorkflow).toContain('if (noActiveProfilesPayload) process.exit(0)')
    expect(sourceRefreshWorkflow).toContain("if (response.status !== 200 && response.status !== 207)")
    expect(sourceRefreshWorkflow).toContain('deliveryImpactingFailure')
    expect(sourceRefreshWorkflow).toContain('failedRequired')
    expect(sourceRefreshWorkflow).toContain('criticality')
    expect(sourceRefreshWorkflow).toContain('invalid or unparseable payload')
