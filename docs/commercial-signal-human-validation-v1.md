...
- A corrected human label is a new reviewer revision with reviewer, reason and timestamp.
- A model/LLM identity cannot be registered as a human reviewer.

## Blind review fields

Open `review.html` (or `review.csv`) and judge the opportunity only from the agency profile, anonymized company continuity key, factual model-input snapshot, evidence dates, source/provenance and corporate URLs.
...
For a double-reviewed subset, use different human `reviewer_id` values. If the latest independent reviewers disagree on `(review_label, actionable)`, the sample has no final gold label until a human adjudicator adds `adjudication=true`. Quality v2 is never a reviewer.
...
- `manifest.json` — scope/version/fingerprint/sampling counts;
- `frozen.jsonl` — audit dataset containing hidden model outputs and evidence lineage;
- `review.html` / `review.csv` / `review.json` — model-blind review surface;
...
The report reuses `commercial-signal-evaluation-v2` for Precision@5, Precision@10, NDCG@10, ranking changes, false-positive/false-negative taxonomy and temporal safeguards. It adds human actionable/evidence rates and reviewer agreement. Industry/role-family/region/company-type segment claims remain `unsupported` until those dimensions exist as canonical, safely frozen company-level fields in the exact evaluated lineage; Stage 2 does not manufacture them from agency preferences.
