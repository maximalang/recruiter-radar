...
| `OPPORTUNITY_SCORING_V3_ENABLED` | Включает аддитивный tenant-scoped Phase 7 Opportunity Candidate shadow build только при точном `true`; по умолчанию `false`, apply требует одновременно workspace и organization, Quality не зависит от missing contact, а существующие Opportunity/Today/delivery/readers не переключаются |
| `QUERY_PLANNER_V2_ENABLED` | Включает аддитивный profile-scoped Phase 8 Query Planner build только при точном `true`; по умолчанию `false`, apply требует одновременно workspace и client profile, exclusions/feedback/ranking остаются раздельными, unresolved geography не исполняется, а существующий ingest/readers не переключаются |
