...
- `company-newsrooms`: set `COMPANY_NEWSROOMS_TARGETS_FILE` for reviewed newsroom URLs, `COMPANY_NEWSROOMS_INPUT_FILE` for snapshots, or provider env. Controlled check: `npm run source:fetch:company-newsrooms`; context-only.
