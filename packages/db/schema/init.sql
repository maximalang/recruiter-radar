...
CREATE INDEX account_login_challenges_user_created_idx
  ON account_login_challenges (user_id, created_at DESC);
CREATE INDEX account_login_challenges_request_created_idx
  ON account_login_challenges (request_key_hash, created_at DESC);
CREATE INDEX account_login_challenges_expires_idx
  ON account_login_challenges (expires_at);
CREATE INDEX account_login_challenges_created_idx
...
-- Canonical strong-key entity-resolution mapping shared by digest assembly and
-- cross-fragment suppression. Company names never merge entities; platform/ATS
-- hosts are excluded from domain identity.
CREATE VIEW org_corroboration_keys_v1 AS
...
-- Bounded, tenant-scoped suppression propagated across ER-equivalent fragments.
-- No FKs by design: the audit snapshot survives org merges and candidate cleanup.
CREATE TABLE client_org_suppressions (
  id BIGSERIAL PRIMARY KEY,
...
CREATE UNIQUE INDEX client_org_suppressions_profile_key_uidx
  ON client_org_suppressions (client_profile_id, suppression_key);

CREATE INDEX client_org_suppressions_profile_until_idx
  ON client_org_suppressions (client_profile_id, suppressed_until);

CREATE INDEX client_org_suppressions_until_idx
