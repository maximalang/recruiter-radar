...
const { Pool } = pg
const ADVISORY_LOCK_KEY = 2_026_091_300
...
try {
  ...
  if (!lockAcquired) {
    ...
    const before = await countEvents(client, config.retentionDays)
    if (mode === 'dry-run') {
      report = {
        ok: true,
        mode,
      ...
      const deletion = await deleteEvents(client, config)
      const after = await countEvents(client, config.retentionDays)
      report = {
        ok: true,
        mode,
        retentionDays: config.retentionDays,
...
function readConfiguration() {
  ...
  return {
    ...
    retentionDays: parseBoundedInteger(
      process.env.PRODUCT_TELEMETRY_RETENTION_DAYS,
      730,
      30,
      3_650,
      'PRODUCT_TELEMETRY_RETENTION_DAYS',
    ),
    batchSize: parseBoundedInteger(
      process.env.PRODUCT_TELEMETRY_CLEANUP_BATCH_SIZE,
      500,
      1,
      10_000,
      'PRODUCT_TELEMETRY_CLEANUP_BATCH_SIZE',
    ),
    maxBatches: parseBoundedInteger(
      process.env.PRODUCT_TELEMETRY_CLEANUP_MAX_BATCHES,
      20,
      1,
      1_000,
      'PRODUCT_TELEMETRY_CLEANUP_MAX_BATCHES',
    ),
  }
}

async function countEvents(connection, retentionDays) {
  const result = await connection.query(
    `SELECT
       COUNT(*)::BIGINT AS scanned,
       COUNT(*) FILTER (
         WHERE occurred_at < clock_timestamp() - MAKE_INTERVAL(days => $1)
       )::BIGINT AS eligible
...
async function deleteEvents(connection, config) {
  let deleted = 0
  let batches = 0
  while (batches < config.maxBatches) {
    const result = await connection.query(
      `WITH candidates AS (
       ...
       removed AS (
         DELETE FROM product_telemetry_events AS event
         USING candidates
