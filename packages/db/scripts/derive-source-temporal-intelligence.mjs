...
export async function deriveSourceTemporalIntelligence({ connectionString = process.env.DATABASE_URL, now = new Date() } = {}) {
  ...
  try {
    const rows = (await client.query(`SELECT signal.org_id, signal.source, signal.signal_type::TEXT, signal.headline, signal.payload, signal.updated_at, ARRAY_AGG(DISTINCT lineage.evidence_id) FILTER (WHERE lineage.evidence_id IS NOT NULL) AS evidence_ids FROM signals signal LEFT JOIN source_signal_evidence_lineage_v1 lineage ON lineage.signal_id=signal.id AND lineage.organization_id=signal.org_id WHERE signal.signal_type <> 'job_posting' AND signal.updated_at BETWEEN $1::TIMESTAMPTZ - INTERVAL '30 days' AND $1::TIMESTAMPTZ GROUP BY signal.id`, [now.toISOString()])).rows;
    const vacancyRows = (await client.query(`SELECT vacancy.organization_id AS org_id, vacancy.normalized_role AS headline, vacancy.location, vacancy.active, vacancy.source_families, vacancy.evidence_ids, COALESCE(ARRAY_AGG(DISTINCT signal.payload->>'department') FILTER (WHERE vacancy.active AND NULLIF(signal.payload->>'department','') IS NOT NULL), ARRAY[]::TEXT[]) AS departments, COALESCE(JSONB_AGG(DISTINCT JSONB_BUILD_OBJECT('id',event.id::TEXT,'type',event.event_type,'role',vacancy.normalized_role,'occurredAt',event.occurred_at)) FILTER (WHERE event.id IS NOT NULL), '[]'::JSONB) AS lifecycle_events FROM canonical_vacancies_v1 vacancy LEFT JOIN canonical_vacancy_publications_v1 publication ON publication.canonical_vacancy_id=vacancy.id LEFT JOIN signals signal ON signal.id=publication.signal_id LEFT JOIN canonical_vacancy_events_v1 event ON event.canonical_vacancy_id=vacancy.id AND event.occurred_at >= $1::TIMESTAMPTZ - INTERVAL '31 days' WHERE vacancy.active OR vacancy.updated_at >= $1::TIMESTAMPTZ - INTERVAL '31 days' GROUP BY vacancy.id`, [now.toISOString()])).rows;
    const groups = groupRows(rows, vacancyRows);
...
function groupRows(rows, vacancyRows) { const map = new Map(); for (const row of rows) { const subjectType = SUBJECT_BY_SOURCE.get(row.source); if (!subjectType) continue; const key = `${row.org_id}:${subjectType}`; const item = map.get(key) ?? { orgId: row.org_id, subjectType, sourceFamily: row.source, rows: [], evidenceIds: [] }; item.rows.push(row); item.evidenceIds.push(...(row.evidence_ids ?? [])); item.evidenceIds = [...new Set(item.evidenceIds.map(Number))]; map.set(key, item); } for (const row of vacancyRows) { const key = `${row.org_id}:vacancies`; const item = map.get(key) ?? { orgId: row.org_id, subjectType: 'vacancies', sourceFamily: 'canonical-vacancy-lifecycle', rows: [], evidenceIds: [] }; item.rows.push(row); item.evidenceIds.push(...(row.evidence_ids ?? [])); item.evidenceIds = [...new Set(item.evidenceIds.map(Number))]; map.set(key, item); } return [...map.values()]; }
function buildMetrics(type, rows) { if (type === 'vacancies') { const active=rows.filter((x)=>x.active); const events=rows.flatMap((x)=>Array.isArray(x.lifecycle_events)?x.lifecycle_events:[]); return { current_count: active.length, roles: unique(active.map((x)=>x.headline)), geographies: unique(active.map((x)=>x.location)), departments: unique(active.flatMap((x)=>x.departments??[])), lifecycle_events: [...new Map(events.map((x)=>[String(x.id),x])).values()] } } if (type === 'fns_company') return latest(rows, { headcount: ['employee_count','headcount'], revenue: ['revenue'], support_count: ['support_count'] }); if (type === 'government_procurement') return { contract_count: rows.length, aggregate_value: rows.reduce((n,x)=>n+num(x.payload?.contract_value ?? x.payload?.value),0), customers: unique(rows.map((x)=>x.payload?.customer_name)), regions: unique(rows.map((x)=>x.payload?.region)) }; return { application_count: rows.filter((x)=>/application/i.test(x.payload?.status ?? '')).length, registration_count: rows.filter((x)=>/registration|registered/i.test(x.payload?.status ?? '')).length } }
